...
/*!
    \group qthldplugincdbext
    \title Qt Hldplugin CDB Extensions

    Qt Hldplugin CDB extensions are loaded into CDB.exe (see cdbengine.cpp).
...
// so that the calling CDB engine in Qt Hldplugin can match the callback.
...
// Report back PID so that Qt Hldplugin engine knows whom to DebugBreakProcess.
extern "C" HRESULT CALLBACK pid(CIDebugClient *client, PCSTR args)
{
    ...
    if (token == 0) // partial message
        return S_OK;
    dprintf("Qt Hldplugin CDB extension version 4.3 %d bit.\n",
            sizeof(void *) * 8);
...
extern "C" HRESULT CALLBACK script(CIDebugClient *client, PCSTR argsIn)
{
    ...
    ExtensionContext::instance().report('N', token, 0, "script",
            ...
            "You need to define PYTHON_INSTALL_DIR in your hldplugin build environment "
...
// Report stop of debuggee to Hldplugin. This is hooked up as .idle_cmd command
// by the Hldplugin engine to reliably get notified about stops.
...
extern "C" HRESULT CALLBACK help(CIDebugClient *, PCSTR)
{
    ...
    str << "### Qt Hldplugin CDB extension" << "\n\n";

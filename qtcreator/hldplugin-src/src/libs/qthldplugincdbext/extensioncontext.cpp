...
{
    if (!client || m_hookedClient || m_hldpluginEventCallback)
        return;
    ...
    if (client->GetEventCallbacks(&m_oldEventCallback) == S_OK) {
        m_hldpluginEventCallback = new EventCallback(m_oldEventCallback);
        client->SetEventCallbacks(m_hldpluginEventCallback);
    }
    if (client->GetOutputCallbacksWide(&m_oldOutputCallback) == S_OK) {
        m_hldpluginOutputCallback = new OutputCallback(m_oldOutputCallback);
        client->SetOutputCallbacksWide(m_hldpluginOutputCallback);
    }
}

void ExtensionContext::startRecordingOutput()
{
    if (m_hldpluginOutputCallback)
        m_hldpluginOutputCallback->startRecording();
    else
...
{
    return m_hldpluginOutputCallback ? m_hldpluginOutputCallback->stopRecording() : std::wstring();
}
...
{
    if (!m_hookedClient || (!m_hldpluginEventCallback && !m_hldpluginOutputCallback))
        return;

    if (m_hldpluginEventCallback) {
        m_hookedClient->SetEventCallbacks(m_oldEventCallback);
        delete m_hldpluginEventCallback;
        m_hldpluginEventCallback = 0;
        m_oldEventCallback = 0;
    }

    if (m_hldpluginOutputCallback) {
        m_hookedClient->SetOutputCallbacksWide(m_oldOutputCallback);
        delete m_hldpluginOutputCallback;
        m_hldpluginOutputCallback = 0;
...
bool ExtensionContext::report(char code, int token, int remainingChunks, const char *serviceName, PCSTR Format, ...)
{
    if (!isInitialized())
        return false;
    // '<qthldplugincdbext>|R|<token>|<serviceName>|<one-line-output>'.
    m_control->Output(DEBUG_OUTPUT_NORMAL, "<qthldplugincdbext>|%c|%d|%d|%s|",
...
/*!  \class ExtensionCommandContext
    ...
    Provides the IDebug objects on demand. \ingroup qthldplugincdbext
*/

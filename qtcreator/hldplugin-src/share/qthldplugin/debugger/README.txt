...
to provide what Qt Hldplugin needs, it can be used in a plain GDB and LLDB
session, too.

With

        python import sys
        python sys.path.insert(1, '<path/to/qthldplugin>/share/qthldplugin/debugger/')
...
Or for LLDB (.lldbinit or directly in the LLDB interpreter):
      ...
      command script import <path/to/qthldplugin>/share/qthldplugin/debugger/lldbbridge.py

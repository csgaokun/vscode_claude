...
The build artifacts can be downloaded from GitHub and be installed into an existing Qt Hldplugin
installation.
...
The value for `QT_HLDPLUGIN_VERSION` specifies the Qt Hldplugin version to use for building the plugin.

The value for `QT_HLDPLUGIN_SNAPSHOT` can either be `NO` or `latest` or the build ID of a specific
snapshot build for the Qt Hldplugin version that you specified.

You need to keep these values updated for different versions of your plugin, and take care
that the Qt version and Qt Hldplugin version you specify are compatible.
...
* Download and unpack the binary for the Qt Hldplugin version

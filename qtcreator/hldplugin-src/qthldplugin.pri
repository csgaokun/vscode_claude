!isEmpty(QTHLDPLUGIN_PRI_INCLUDED):error("qthldplugin.pri already included")
QTHLDPLUGIN_PRI_INCLUDED = 1

include($$PWD/qthldplugin_ide_branding.pri)
!isEmpty(IDE_BRANDING_PRI): include($$IDE_BRANDING_PRI)

PRODUCT_BUNDLE_IDENTIFIER=$${PRODUCT_BUNDLE_ORGANIZATION}.$${IDE_ID}
VERSION = $$QTHLDPLUGIN_VERSION
...
defineReplace(qtLibraryName) {
   ...
   win32 {
      VERSION_LIST = $$split(QTHLDPLUGIN_VERSION, .)
...
osx {
    ...
    # check if IDE_BUILD_TREE is actually an existing Qt Hldplugin.app,
    ...
    IDE_LIBRARY_PATH = $$IDE_OUTPUT_PATH/$$IDE_LIBRARY_BASENAME/qthldplugin
    IDE_PLUGIN_PATH  = $$IDE_LIBRARY_PATH/plugins
    IDE_DATA_PATH    = $$IDE_OUTPUT_PATH/share/qthldplugin
    IDE_DOC_PATH     = $$IDE_OUTPUT_PATH/share/doc/qthldplugin
    IDE_BIN_PATH     = $$IDE_OUTPUT_PATH/bin
    win32: \
        IDE_LIBEXEC_PATH = $$IDE_OUTPUT_PATH/bin
    else: \
        IDE_LIBEXEC_PATH = $$IDE_OUTPUT_PATH/libexec/qthldplugin
    !isEqual(IDE_SOURCE_TREE, $$IDE_OUTPUT_PATH):copydata = 1

    LINK_LIBRARY_PATH = $$IDE_BUILD_TREE/$$IDE_LIBRARY_BASENAME/qthldplugin
    LINK_PLUGIN_PATH  = $$LINK_LIBRARY_PATH/plugins

    INSTALL_LIBRARY_PATH = $$QTC_PREFIX/$$IDE_LIBRARY_BASENAME/qthldplugin
    INSTALL_PLUGIN_PATH  = $$INSTALL_LIBRARY_PATH/plugins
    win32: \
        INSTALL_LIBEXEC_PATH = $$QTC_PREFIX/bin
    else: \
        INSTALL_LIBEXEC_PATH = $$QTC_PREFIX/libexec/qthldplugin
    INSTALL_DATA_PATH    = $$QTC_PREFIX/share/qthldplugin
    INSTALL_DOC_PATH     = $$QTC_PREFIX/share/doc/qthldplugin
...
win32:exists($$IDE_SOURCE_TREE/lib/qthldplugin) {
    # for .lib in case of binary package with dev package
    LIBS *= -L$$IDE_SOURCE_TREE/lib/qthldplugin
    LIBS *= -L$$IDE_SOURCE_TREE/lib/qthldplugin/plugins
}
...
LIBS *= -L$$LINK_LIBRARY_PATH  # Qt Hldplugin libraries
...
!isEmpty(QTC_PLUGIN_DEPENDS) {
    ...
    LIBS *= -L$$LINK_PLUGIN_PATH  # when output path is different from Qt Hldplugin build directory
}

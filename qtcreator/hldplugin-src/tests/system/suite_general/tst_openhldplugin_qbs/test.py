...
def main():
    pathHldplugin = os.path.join(srcPath, "hldplugin", "qthldplugin.qbs")
    if not neededFilePresent(pathHldplugin):
        return

    startQC()
    if not startedWithoutPluginError():
        return
    openQbsProject(pathHldplugin)
    ...
    rootNodeTemplate = "{column='0' container=':Qt Hldplugin_Utils::NavigationTreeView' text~='%s( \[\S+\])?' type='QModelIndex'}"
    ntwObject = waitForObject(rootNodeTemplate % "Qt Hldplugin", 200000)
    ...
    compareProjectTree(rootNodeTemplate % "Qt Hldplugin", "projecttree_hldplugin.tsv")
    ...
    if deprecationWarnings:
        test.warning("Hldplugin claims that the .qbs file uses deprecated features.",

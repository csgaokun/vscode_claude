...
static const char QtHldpluginExecutable[] = "qthldplugin";
...
public:
    CrashHandlerPrivate(pid_t pid,
        ...
        : pid(pid),
          hldpluginInPath(Utils::Environment::systemEnvironment().searchInPath(QLatin1String(QtHldpluginExecutable))),
          dialog(crashHandler, signalName, appName) {}

    const pid_t pid;
    const Utils::FilePath hldpluginInPath; // Backup debugger.
...
void CrashHandler::debugApplication()
{
    ...
    // Prepare command.
    QString executable = d->hldpluginInPath.toString();

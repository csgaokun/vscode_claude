...
static void printErrorAndExit()
{
    ...
    err << QString::fromLatin1("This crash handler will be called by Qt Hldplugin itself. "
                               "Do not call this manually.\n");
    exit(EXIT_FAILURE);
}

static bool hasProcessOriginFromQtHldpluginBuildDir(Q_PID pid)
{
    const QString executable = QFile::symLinkTarget(QString::fromLatin1("/proc/%1/exe")
        .arg(QString::number(pid)));
    return executable.contains("qthldplugin");
}
...
int main(int argc, char *argv[])
{
    ...
    if (!hasProcessOriginFromQtHldpluginBuildDir(parentPid))
        printErrorAndExit();

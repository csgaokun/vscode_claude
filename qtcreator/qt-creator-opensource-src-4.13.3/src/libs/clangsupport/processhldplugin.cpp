...
void ProcessHldplugin::setTemporaryDirectoryPattern(const QString &temporaryDirectoryPattern)
{
    m_temporaryDirectoryPattern = temporaryDirectoryPattern;
    resetTemporaryDirectory();
}

void ProcessHldplugin::setProcessPath(const QString &processPath)
{
    m_processPath = processPath;
}

void ProcessHldplugin::setArguments(const QStringList &arguments)
{
    m_arguments = arguments;
}

void ProcessHldplugin::setEnvironment(const Utils::Environment &environment)
{
    m_environment = environment;
}

std::future<QProcessUniquePointer> ProcessHldplugin::createProcess() const
{
...
void ProcessHldplugin::setObserver(QObject *observer)
{
    this->m_observer = observer;
}

void ProcessHldplugin::checkIfProcessPathExists() const
{
    if (!QFileInfo::exists(m_processPath)) {
        const QString messageTemplate = QCoreApplication::translate("ProcessHldplugin",
                                                                    "Executable does not exist: %1");
        throwProcessException(messageTemplate.arg(m_processPath));
    }
}

void ProcessHldplugin::checkIfProcessWasStartingSuccessful(QProcess *process) const
{
    if (process->exitStatus() == QProcess::CrashExit || process->exitCode() != 0)
        dispatchProcessError(process);
}

void ProcessHldplugin::dispatchProcessError(QProcess *process) const
{
    switch (process->error()) {
        case QProcess::UnknownError: {
            const QString message = QCoreApplication::translate("ProcessHldplugin",
...
void ProcessHldplugin::postProcessStartedEvent() const
{
    if (m_observer)
        QCoreApplication::postEvent(m_observer, new ProcessStartedEvent);
}

void ProcessHldplugin::throwProcessException(const QString &message) const
{
    postProcessStartedEvent();
    throw ProcessException(message);
}

const QTemporaryDir &ProcessHldplugin::temporaryDirectory() const
{
    return *m_temporaryDirectory.get();
}

void ProcessHldplugin::resetTemporaryDirectory()
{
    m_temporaryDirectory = std::make_unique<Utils::TemporaryDirectory>(m_temporaryDirectoryPattern);
}

QProcessEnvironment ProcessHldplugin::processEnvironment() const
{

...
static bool dumpQDateTime(const SymbolGroupValue &v, std::wostream &str, std::string *encoding)
{
    ...
    if (QtInfo::get(v.context()).version == 5) {
        // the dumper on the hldplugin side expects msecs/spec/offset/tz/status/type info version
...
{
    struct HldpluginImageHeader { // Header for image display as edit format, followed by data.
        int width;
        int height;
        int format;
    };
    ...
    str << header.width << L'x' << header.height << L", depth: " << depth
        ...
        << nbytes << L" bytes";
    delete [] qImageData;
    // Create Hldplugin Image data for display if reasonable size
    if (memoryHandle && data && nbytes > 0 && nbytes < 205824) {
        if (unsigned char *imageData = SymbolGroupValue::readMemory(v.context().dataspaces, data, nbytes)) {
            unsigned char *hldpluginImageData = new unsigned char[sizeof(HldpluginImageHeader) + nbytes];
            memcpy(hldpluginImageData, &header, sizeof(HldpluginImageHeader));
            memcpy(hldpluginImageData + sizeof(HldpluginImageHeader), imageData, nbytes);
            delete [] imageData;
            *memoryHandle = new MemoryHandle(hldpluginImageData, sizeof(HldpluginImageHeader) + nbytes);
            // cppcheck: don't delete[] hldpluginImageData here, it's taken over by MemoryHandle
        }
    }
    return true;
}

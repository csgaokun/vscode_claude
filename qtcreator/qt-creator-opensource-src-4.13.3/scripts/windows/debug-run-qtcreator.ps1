...
if ([string]::IsNullOrWhiteSpace($ExePath)) {
    $installedExe = Join-Path $BuildRoot 'install\qt-creator\bin\qtcreator.exe'
    $buildExe = Join-Path $BuildRoot 'build\bin\qtcreator.exe'
...
Invoke-LoggedCommand -StepName 'Run Qt Creator' -LogPath $runtimeLog -Command $cmd

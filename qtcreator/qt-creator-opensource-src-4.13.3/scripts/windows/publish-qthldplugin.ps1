...
$installDir = Join-Path $BuildRoot 'install\qt-hldplugin'
...
$installedExe = Join-Path $installDir 'bin\qthldplugin.exe'
$buildExe = Join-Path $buildDir 'bin\qthldplugin.exe'
$qthldpluginExe = if (Test-Path -LiteralPath $installedExe) { $installedExe } else { $buildExe }
$qmakeExe = Join-Path $QtRoot 'bin\qmake.exe'

Assert-FileExists -Path $deployScript
Assert-FileExists -Path $qthldpluginExe
...
$deployCmd = "cd /d `"$buildDir`" && python -u `"$deployScript`" -i `"$qthldpluginExe`" `"$qmakeExe`""
Invoke-LoggedCommand -StepName 'Deploy Qt runtime' -LogPath $logDeploy -Command $deployCmd

$packageZip = Join-Path $artifactsDir ("qthldplugin-msvc2017-qt5.12.12-{0}.zip" -f $BuildType.ToLowerInvariant())

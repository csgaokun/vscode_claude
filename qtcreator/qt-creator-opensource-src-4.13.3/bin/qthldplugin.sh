...
# libraries that Qt Hldplugin depends on.
...
# Add Qt Hldplugin library path
_ORIGINAL_LD_LIBRARY_PATH=$LD_LIBRARY_PATH
LD_LIBRARY_PATH=$libdir:$libdir/qthldplugin$qtlibpath${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}
export LD_LIBRARY_PATH
exec "$bindir/qthldplugin" -user-library-path "$_ORIGINAL_LD_LIBRARY_PATH" ${1+"$@"}

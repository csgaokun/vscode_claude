...
The Qt Design Studio Manual is based on the Qt Hldplugin Manual, with additional
topics. When building the Qt Design Studio Manual, parts of the Qt Hldplugin
Manual are pulled in from Qt Hldplugin sources. This is enabled by creating
...
version, you must run `qmake -r` on `qthldplugin.pro` with the `IDE_BRANDING_PRI`
option set to the absolute path of `ide_branding.pri` in the Qt Design Studio
repository.

For example, on Windows enter (all on one line):

`C:\dev\qtc-super\qthldplugin>..\..\..\Qt\5.14.1\msvc2017_64\bin\qmake.exe
    qthldplugin.pro -r
...
1. Run `qmake` from Qt 5.14.0, or later with the path to the branding
   ...
   `<relative_path_to>\qmake.exe qthldplugin.pro -r IDE_BRANDING_PRI=<absolute_path_to>\tqtc-plugin-qtquickdesigner\studiodata\branding\ide_branding.pri`
5. Run `make docs` on Linux and macOS or `nmake docs` on Windows.

The docs are generated in `qthldplugin\doc\html\qtdesignstudio` with the

...
def startQC(additionalParameters=None, withPreparedSettingsPath=True, closeLinkToQt=True, cancelTour=True):
    global SettingsPath
    appWithOptions = ['"Qt Hldplugin"' if platform.system() == 'Darwin' else "qthldplugin"]
    ...
    if closeLinkToQt:
        clickButton(waitForObject(":*Qt Hldplugin.Do Not Show Again_QToolButton"))
    if cancelTour:
        clickButton(waitForObject(":*Qt Hldplugin.Do Not Show Again_QToolButton"))
    return appContext;
...
def startedWithoutPluginError():
    try:
        ...
        test.fatal("Could not perform clean start of Qt Hldplugin - Plugin error occurred.",
...
def waitForCleanShutdown(timeOut=10):
    ...
    if platform.system() in ('Windows','Microsoft'):
        ...
        if platform.system() == 'Linux' and JIRA.isBugStillOpen(15749):
            pgrepOutput = getOutputFromCmdline(["pgrep", "-f", "qthldplugin_process_stub"],
                                               acceptedError=1)
            pids = pgrepOutput.splitlines()
            if len(pids):
                print("Killing %d qthldplugin_process_stub instances" % len(pids))
            for pid in pids:
                try:
...
def __removeTestingDir__():
    ...
    devicesXML = os.path.join(tmpSettingsDir, "QtProject", "qthldplugin", "devices.xml")
...
def substituteTildeWithinToolchains(settingsDir):
    toolchains = os.path.join(settingsDir, "QtProject", 'qthldplugin', 'toolchains.xml')
...
def substituteDefaultCompiler(settingsDir):
    ...
    if compiler:
        qtversion = os.path.join(settingsDir, "QtProject", 'qthldplugin', 'qtversion.xml')
...
def substituteCdb(settingsDir):
    def canUse32bitCdb():
        try:
            ...
            autLine = filter(lambda line: "AUT/qthldplugin" in line, serverIni.splitlines())[0]
            autPath = autLine.split("\"")[1]
            return os.path.exists(os.path.join(autPath, "..", "lib", "qthldplugincdbext32"))
        except:
    ...
    if canUse32bitCdb():
        ...
        architecture = "x64"
        bitness = "64"
    debuggers = os.path.join(settingsDir, "QtProject", 'qthldplugin', 'debuggers.xml')
...
# current dir is directory holding qthldplugin.py
